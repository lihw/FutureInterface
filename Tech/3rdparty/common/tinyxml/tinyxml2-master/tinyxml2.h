...
#if defined(ANDROID_NDK) || defined(__BORLANDC__) || defined(ANDROID)
#   include <ctype.h>
#   include <limits.h>
#   include <stdio.h>
#   include <stdlib.h>
...
public:
    ...
    ~DynArray() {
        if ( _mem != _pool ) {
            //delete [] _mem;
            // protoss: replace delete with delete for protoss compatiblility.
            free(_mem);
        }
    }
...
private:
    void EnsureCapacity( int cap ) {
        if ( cap > _allocated ) {
            ...
            // protoss: we replace new with malloc as protoss overrides the global new operator.
            T* newMem = (T*)malloc(sizeof(T) * newAllocated);
            memcpy( newMem, _mem, sizeof(T)*_size );	// warning: not using constructors, only works for PODs
            if ( _mem != _pool ) {
                //delete [] _mem;
                // protoss: we replace delete with free as protoss overrides the global new operator.
                free(_mem);
            }
            _mem = newMem;
...
public:
    ...
    ~MemPoolT() {
        ...
        for( int i=0; i<_blockPtrs.Size(); ++i ) {
            //delete _blockPtrs[i];
            // protoss: replace delete with free
            free(_blockPtrs[i]);
        }
    }
    ...
    virtual void* Alloc() {
        if ( !_root ) {
            ...
            // replace new with malloc to make tinyxml2 compatible with protoss.
