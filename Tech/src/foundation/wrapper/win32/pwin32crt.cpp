...
{
    PASSERT(dst != src);
    // dst should have at least n + 1 chars.
...
{
    return str == P_NULL? 0 : (puint32)strlen(str);
}
...
void P_APIENTRY pstrcat(pchar* dst, const pchar* src)
{
    ...
    pstrncpy(&dst[dstLen], src, srcLen);
}
...
pbool P_APIENTRY pfeof(PFile* file)
{
